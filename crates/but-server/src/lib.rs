...
pub async fn run(config: Config) -> anyhow::Result<()> {
    ...
    let app = app
        ...
        .route(
            "/apply_unity_conflict_resolution",
            but_post_async(legacy::virtual_branches::apply_unity_conflict_resolution_cmd),
        )
        .route(
    ...
    match command {
        ...
        "apply_unity_conflict_resolution" => {
            ...
            match params {
                Ok(params) => {
                    let result =
                        legacy::virtual_branches::apply_unity_conflict_resolution_cmd(params).await;
                    result.map(|r| json!(r))
                }
                Err(e) => Err(e),
            }
